...
public class S3Writer extends Writer {
    public static class Job extends Writer.Job {
        ...
        private void validateParameter() {
            ...
            if (StringUtils.isNotBlank(compress)) {
                String errorMessage = String.format(
                        "S3写暂时不支持压缩, 该压缩配置项[%s]不起效用", compress);
        ...
        public void prepare() {
            ...
            // warn: if the bucket does not exist, then create it.
            try {
                ...
                if ("truncate".equals(writeMode)) {
                    ...
                    while (true) {
                        ...
                        ObjectListing listing = this.s3Client.listObjects(bucket, object);
                        List<S3ObjectSummary> objectSummaries = listing
                                .getObjectSummaries();
                        for (S3ObjectSummary objectSummary : objectSummaries) {
                            LOG.info(String.format("delete S3 object [%s].",
                                    objectSummary.getKey()));
                            this.s3Client.deleteObject(bucket,
                                    objectSummary.getKey());
                        }
                    ...
                    if (0 < listing.getObjectSummaries().size()) {
                        ...
                        objectKeys.append("[ ");
                        for (S3ObjectSummary s3ObjectSummary : listing
                                .getObjectSummaries()) {
        ...
        public List<Configuration> split(int mandatoryNumber) {
            ...
            try {
                List<S3ObjectSummary> s3ObjectSummaries = this.s3Client
            ...
            for (int i = 0; i < mandatoryNumber; i++) {
                ...
                Configuration splitTaskConfig = this.writerSliceConfig
                        .clone();
                ...
                splitTaskConfig.set(Key.OBJECT, fullObjectName);

                LOG.info(String.format("split write object name:[%s]",
                        fullObjectName));
    ...
    public static class Task extends Writer.Task {
            ...
            try {
                ...
                while ((record = lineReceiver.getFromReader()) != null) {
                    ...
                    if (needInitMultipartTransform) {
                        ...
                        LOG.info(String
                                .format("write to bucket: [%s] object: [%s] with S3 uploadId: [%s]",
                                        this.bucket, currentObject,
         ...
         * 如果你用同一个part号码，上传了新的数据，那么S3上已有的这个号码的Part数据将被覆盖。
         *
         * @throws Exception
         */
