...
S3Writer提供了向Amazon S3兼容对象存储写入类CSV格式的一个或者多个表文件。
...
* **endpoint**
	...
	* 描述：S3 Server的EndPoint地址，例如https://s3.yourdomain.com。
...
* **object**
 	...
 	* 描述：S3Writer写入的文件名，S3使用文件名模拟目录的实现。 <br />
		...
		使用"object": "/cdo/datax"，写入的object以/cdo/datax开头，后缀随机添加字符串，/作为S3模拟目录的分隔符。
